...
- **Quick filters:** The Filters action opens a single-column editor for ordinary source-row conditions. Field, comparison, value, and Remove carry the primary hierarchy; the existing advanced Query Builder remains available from a clearly labeled action. Opening the editor never mutates the draft. Nested predicates, computed references, field-to-field comparisons, and unrelated unapplied query changes open the full builder so their meaning remains visible.
