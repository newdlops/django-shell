...
- **Sorting:** Sort headers are buttons with `aria-sort`. Activating a header applies asc/desc/default order immediately to the currently applied rows Recipe, without submitting unrelated Query Builder draft edits. Sort order is communicated by text or accessible name as well as an icon, and duplicate activations are disabled while rows reload.
...
- **Quick filters:** The Filters action opens a single-column condition composer for Django developers inspecting live rows. Keep field, comparison, and typed value on one readable line; show AND/OR at the group header and between siblings. Nested groups, exclusion, duplicate, field comparisons, and relative dates belong in this surface. Computed references, existence subqueries, and unrelated unapplied query changes open the full builder. Opening the editor never mutates the draft.
- **Property filters:** List root `@property` and cached properties under Model properties. Give their values explicit Text, Number, and Boolean types. Support direct root AND conditions in Rows mode, with inline guidance for unsupported contexts. Declared `djshell_annotations` use SQL; other getters run on DB-filtered candidates before pagination. Count and lazy property reads must use the same filter. Explain the scan cost without implying that only the visible page is filtered.
- **Quick-filter interaction:** Keep the grid visible, retain the shared revisioned draft and Apply lifecycle, and keep one Apply action in the existing footer. Clear row filters edits only the source-row predicate and remains undoable. Selecting a field moves focus to its typed value; closing returns focus to Filters. At narrow widths, fields wrap above comparison/value controls without horizontal scrolling. Loading metadata, inline validation, disabled Apply, server rejection, long field names, light/dark/high-contrast themes, and keyboard operation are explicit acceptance checks.
- **Relationship filters:** A compact field-path button opens a searchable field explorer with a model breadcrumb and separate relationship/field lists. Developers can drill into `company` then `user`, or paste `company__user__email__icontains`; only metadata-validated field paths and allowed comparisons are accepted. Keep `company_id` distinct from `company`. Preserve navigation and search across loading/retry, allow returning to any ancestor, and focus the typed value after selection.
- **Filter composer contract (Operate):** Use existing UI fonts for controls and editor fonts for field paths. Reuse VS Code neutral surfaces, 1px structural borders, 4px control radii, and 4/8/12px spacing. Group depth is expressed by indentation and a neutral guide, not nested cards. Add condition/group stays beside each group's children; one Apply remains in the footer above the grid. At 390px controls wrap and the explorer clamps inside the viewport; at 768/1440px paths share a line with comparisons and values. Keep focus rings, keyboard search/Enter/Escape, no-match guidance, pending/error/retry, typed list/range/date values, exclusion, undo, long paths, and disabled limits explicit. No decorative motion or added dependencies. Acceptance: build a nested AND/OR relation filter, duplicate/exclude a clause, edit a parent path, paste a lookup, recover a failed child load, and reopen the same draft without losing meaning or values.
...
- **Draft and applied state:** Query Builder editing changes only the draft. Reset restores the applied Recipe and Clear creates an empty draft; neither executes a query. Apply runs a revisioned snapshot. Direct grid-header sorting is the explicit lightweight exception: it revises only the currently applied Recipe's outer order and preserves an unrelated dirty draft. If the user edits during execution, the newer draft remains marked `Draft` while the grid reflects the successful snapshot.
