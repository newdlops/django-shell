...
interface DapMessage { body?: unknown; command?: string; event?: string; message?: string; request_seq?: number; seq?: number; success?: boolean; type: "event" | "request" | "response"; }
...
interface StoppedEventBody { reason?: string; threadId?: number; }
interface ContinuedEventBody { allThreadsContinued?: boolean; threadId?: number; }
...
export class DirectDebugAdapterSession implements DebugRequestSession {
  ...
  customRequest<T = unknown>(command: string, args: unknown = {}): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const timer = setTimeout(() => { const error = new Error(`DAP request timed out: ${command}`); this.pending.delete(seq); reject(error); if (!this.attached) { this.closeTransport(error); } }, timeoutMs);
  ...
  /** Disconnects from debugpy and closes the transport. */
  async disconnect(): Promise<void> {
    if (!this.socket) { return; }
    this.closeTransport(new Error("Debug adapter disconnected."));
  }
  ...
  private openSocket(endpoint: DebugpyEndpoint): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      socket.once("connect", () => { socket.off("error", fail); this.socket = socket; this.connected = true; this.attached = false; resolve(); });
      socket.once("error", fail);
      socket.on("data", (chunk) => this.handleData(chunk));
      socket.on("error", (error) => this.logger?.log("debug.direct.socket.error", { error: error.message }));
      socket.on("close", () => { this.connected = false; this.attached = false; this.rejectPending(new Error("Debug adapter socket closed.")); this.hooks.onTerminated?.(); });
    });
  }
  ...
  private handleResponse(message: DapMessage): void {
    const requestSeq = message.request_seq ?? 0;
