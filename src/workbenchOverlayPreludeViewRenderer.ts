    ...
    function __dsoApplyPreludeView(root, editor, model, startLine) {
      ...
      try {
        if (editor && editor.updateOptions) {
          const submitMode = root && root.__dsoExecutionMode === "submit";
          editor.updateOptions({ glyphMargin: !submitMode, lineDecorationsWidth: 0, lineNumbers: submitMode ? "on" : function (line) { return __dsoPromptForLine(model, startLine, line, root); }, lineNumbersMinChars: submitMode ? 3 : 1 });
        }
      } catch (eLineNumbers) {}
    }
