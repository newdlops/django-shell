...
- **Quick filters.** Click **Filters** for field → comparison → value editing while keeping the grid visible. **Add condition**, **Match all / any**, **Remove**, and **Clear row filters** edit an undoable draft; **Apply filters** or `Cmd/Ctrl+Enter` applies it. **Advanced query builder** exposes nested groups and computed comparisons. Complex conditions or unrelated unapplied query changes open the full builder automatically.
- **Query Builder.** The model-only **Query Builder** keeps the applied query in a compact summary band and opens a drawer for editing a separate draft. Build nested **All / Any / Not** predicate groups, type-aware values, relation `Exists`, same-row field comparisons (`F`), and relative time values without writing Django syntax. Add ordered computed columns as Aggregate, Scalar Subquery, Exists, Formula / Case / Cast, Window, or restricted Code expression; aliases and paths are validated against live model metadata before Apply. A result filter can target enabled computed aliases after annotation, and **Rows / Summary** makes group-by, ordering, grouped results, and global summaries explicit. The human summary and read-only ORM preview describe the same Recipe before execution.
...
- **Query trust and limits.** Draft edits never alter the grid. **Apply** sends a revisioned snapshot; later draft edits remain visible while that snapshot runs, and a backend rejection preserves the previous grid with actionable inline errors. Empty Recipe means all rows. Query metadata, field paths, lookup names, aliases, function names, nesting, list sizes, and generated ORM text are bounded and allowlisted. A to-many non-Count aggregate is rejected instead of silently changing cardinality; subqueries show their correlation and deterministic ordering; a window alias cannot be used in a result filter.
- **Transport and property limits.** Socket, Auto, Terminal, and ORM transports share Recipe meaning, but the UI reports an unsupported capability rather than changing semantics. Filtering an unannotated Python property requires a bounded full scan and warns before execution; property aggregates require a capable Socket path. The Query Builder does not evaluate raw Python, SQL, or QuerySets while previewing.
- **Sorting.** Click any concrete or enabled computed-column header to apply asc/desc/default order immediately; a pending sort disables duplicate clicks and keeps the existing rows visible until the replacement page succeeds. An unrelated Query Builder draft remains staged and is not submitted. Read-only `@property` columns are excepted; window or non-primary ordering uses bounded OFFSET pagination.
...
After compiling, `node test/filterUx.browser.mjs` checks the production filter webview in an isolated headless Chromium fixture at 390, 768, and 1440 pixels, including keyboard Apply, Undo, typed values, metadata/server errors, and light/high-contrast rendering. It requires Playwright and an installed Chromium; `DJANGO_SHELL_PLAYWRIGHT` may point to a Playwright module and `DJANGO_SHELL_CHROMIUM_EXECUTABLE` to its browser binary. Screenshots overwrite `.vscode-test/results/filter-*.png`. This fixture uses simulated backend replies and does not replace VS Code extension-host or real Django checks.
