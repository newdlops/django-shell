...
async function assertModelQueryBuilderWebview(extension) {
  ...
  try {
    ...
    assert.equal(snapshot.showPickerCalls, 0, "the searchable field explorer handles field selection");
    assert.equal(snapshot.explorerOpen, true);
    assert.equal(snapshot.placeholderVisible, true);
    ...
    assert.equal(snapshot.applyDisabled, true, "the final field explorer draft remains unapplied");
