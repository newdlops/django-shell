...
export function renderWindowBuilder({ dispatch, el, getScope, item, popoverLayer }) {
  ...
  const field = createQuerySelect({ allowEmpty: true, ariaLabel: "Window field", el, onChange: (path) => dispatch({ changes: { field: path ? { kind: "field", path } : undefined }, nodeId: item.nodeId, type: "UPDATE_COMPUTED" }), options: windowFieldOptions(available, "No field"), value: item.field?.path || "" });
  const order = createQuerySelect({ allowEmpty: true, ariaLabel: "Window order field", el, onChange: (path) => dispatch({ changes: { orderBy: path ? [{ direction: "asc", nodeId: `${item.nodeId}-order-1`, ref: { kind: "field", path } }] : [] }, nodeId: item.nodeId, type: "UPDATE_COMPUTED" }), options: windowFieldOptions(available, "Choose order"), value: item.orderBy?.[0]?.ref?.path || "" });
  const partition = createQuerySelect({ allowEmpty: true, ariaLabel: "Partition field", el, onChange: (path) => dispatch({ changes: { partitionBy: path ? [{ kind: "field", path }] : [] }, nodeId: item.nodeId, type: "UPDATE_COMPUTED" }), options: windowFieldOptions(available, "No partition"), value: item.partitionBy?.[0]?.path || "" });
