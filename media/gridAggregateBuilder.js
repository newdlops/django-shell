...
/** Returns schema-backed Aggregate field options; shared selects preserve unavailable values. */
export function aggregateFieldOptions(candidates) {
  const options = [{ label: "All rows", value: "*" }, ...(candidates || []).map((entry) => ({ description: entry.type || "", group: "Fields", label: entry.label || entry.path, value: entry.path }))];
  return options;
}
...
export function renderAggregateBuilder({ dispatch, el, getRecipe, getScope, item, metadata, popoverLayer, validation }) {
  ...
  const fieldPicker = createQuerySelect({ ariaLabel: "Aggregate field", el, onChange: (path) => dispatch({ changes: { field: path === "*" ? { kind: "all" } : { kind: "field", path } }, nodeId: item.nodeId, type: "UPDATE_COMPUTED" }), options: fieldOptions, value: field });
